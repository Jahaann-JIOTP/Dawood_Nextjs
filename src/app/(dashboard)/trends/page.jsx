...
function CustomTrend() {
    ...
    try {
      ...
      const allParameterData = await Promise.all(
        selectedParameters.map(async (parameter) => {
          ...
          const apiUrl = `${baseUrl}/trends?start_date=${startDate}&end_date=${endDate}&meterId=${meterMapping[selectedMeter].slice(0, -1)}&suffixes=${suffixes}`
          ...
          try {
            ...
            if (response.ok) {
              ...
              const dataArr = Array.isArray(result) ? result : result.data || result.payload || []
              const hasNonEmptyData = dataArr.some((entry) => {
                if (entry.data && typeof entry.data === "object") {
                  return Object.keys(entry.data).length > 0
                }
                return Object.keys(entry).some((key) => key !== "timestamp" && entry[key] != null)
              })

              if (hasNonEmptyData) {
                console.log(`✅ Successfully fetched data for ${parameter}`)
                return { parameter, data: dataArr, suffixes, meterPrefix: meterMapping[selectedMeter] }
              }
            }
          } catch (err) {
          ...
          return { parameter, data: [], suffixes, meterPrefix: meterMapping[selectedMeter] }
        }),
      )
      ...
      allParameterData.forEach(({ parameter, data, suffixes, meterPrefix }) => {
        ...
        data.forEach((entry) => {
          ...
          if (entry.data && typeof entry.data === "object" && Object.keys(entry.data).length > 0) {
            ...
            else if (entry.data[suffixes] !== undefined) value = entry.data[suffixes]
            else if (entry.data[meterPrefix + suffixes] !== undefined) value = entry.data[meterPrefix + suffixes]
          ...
          if (value === null) {
            if (entry[fullKey] !== undefined) value = entry[fullKey]
              ...
              const foundKey = keys.find(
                (k) => k.toLowerCase().includes(suffixes.toLowerCase()) || k.includes(suffixes) || k.endsWith(suffixes),
              )
      ...
      const formatted = Array.from(timeMap.values()).sort((a, b) => a.Date - b.Date)

      // Check for valid data
      const hasValidData = formatted.some((d) => {
        return selectedParameters.some((param) => {
          const value = d[param]
          return value !== null && value !== undefined && value !== "null" && !isNaN(Number.parseFloat(value))
        })
      })
      ...
      if (hasValidData) {
        ...
        setChartData([])
        alert(`No valid data found for the selected parameters. Please try different parameters or date range.`)
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (startDate && endDate && selectedMeter && selectedParameters.length > 0) {
      fetchData()
    } else {
      setChartData([])
    }
  ...
  useEffect(() => {
    ...
    try {
      if (valueAxis.renderer.minorGrid && valueAxis.renderer.minorGrid.template) {
  ...
  }, [chartData, isDarkMode, selectedMeter, selectedParameters, startDate, endDate])
      ...
      <div className="relative z-10 h-full flex flex-col">
        ...
        <div className="w-full flex flex-col gap-4 mb-4 sm:mb-6 sm:grid sm:grid-cols-2 lg:grid-cols-4 sm:gap-4 md:gap-6">
          ...
          <div ref={meterDropdownRef} className="relative w-full">
            ...
            >
              <span className="truncate">{selectedMeter || "Select Meter"}</span>
              <span className="float-right mt-0.5">{showMeters ? "▲" : "▼"}</span>
            </button>
            {showMeters && (
          ...
          <div ref={parameterDropdownRef} className="relative w-full">
            ...
            >
              ...
              <span className="float-right mt-0.5">{showParameters ? "▲" : "▼"}</span>
            </button>
          ...
          {chartData.length === 0 && !loading && (
            <div className="absolute inset-0 flex items-center justify-center text-gray-500 dark:text-gray-400 p-4 text-center">
              <div className="text-sm sm:text-base">
                {startDate && endDate && selectedMeter && selectedParameters.length > 0
